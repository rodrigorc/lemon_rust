...
** tracked.
*/
PRIVATE void tplt_xfer(FILE *in, FILE *out, int *lineno)
{
  int iStart;
  char line[LINESIZE];
...
PRIVATE void translate_code(struct lemon *lemp, struct rule *rp){
  ...
  append_str("\n",0,0,0);
  if (rp->lhs->dtnum) {
    append_str("let yyres : ",0,0,0);
    append_str(rp->lhs->datatype,0,0,0);
    append_str(";\n",0,0,0);
  }
  for(i=rp->nrhs; i>0; ){
    i--;
          ...
          if( rp->rhsalias[i] && strcmp(cp,rp->rhsalias[i])==0 ){
            if( cp!=rp->code && cp[-1]=='@' ){
              ...
              ** the token number of X, not the value of X */
              append_str("yyp%d.major,",0,i,0);
            }else{
              append_str("yy%d",0,i,0);
            }
            cp = xp;
            break;
          }
  ...
  if (rp->lhs->dtnum)
      append_str(" YYMinorType::YY%d(yyres)\n",0,rp->lhs->dtnum,0);
  else
      append_str(" YYMinorType::YY0\n",0,0,0);
...
void print_stack_union(
  ...
  */
  for(i=0; i<lemp->nsymbol; i++){
    ...
    if( sp->type==MULTITERMINAL || (sp->datatype==0 && lemp->vartype==0) ){
      sp->dtnum = 0;
      continue;
    }
    cp = sp->datatype;
...
void ReportTable(
  ...
  fprintf(out,"type YYCODETYPE = %s;\n",
    minimum_size_type(-3, lemp->nsymbol+1)); lineno++;
  fprintf(out,"const YYNOCODE: i32 = %d;\n",lemp->nsymbol+1);  lineno++;
  fprintf(out,"type YYACTIONTYPE = %s;\n",
    minimum_size_type(0, lemp->nstate+lemp->nrule+5));  lineno++;
  if( lemp->wildcard ){
    if (lemp->wildcard->datatype) {
        ErrorMsg(lemp->filename, 0,
          "Wildcard token %s must not have a type", lemp->wildcard->name);
        lemp->errorcnt++;
    }
  ...
  fprintf(out,"const YYNSTATE: i32 = %d;\n",lemp->nstate);  lineno++;
  fprintf(out,"const YYNRULE: i32 = %d;\n",lemp->nrule);  lineno++;
  if( lemp->errsym->useCnt ){
    fprintf(out,"const YYERRORSYMBOL: i32 = %d;\n",lemp->errsym->index);  lineno++;
  } else {
    fprintf(out,"const YYERRORSYMBOL: i32 = 0;\n");  lineno++;
  }
  tplt_xfer(in,out,&lineno);
  ...
  */
  ...
  fprintf(out,"fn token_value(t: Token) -> (i32, YYMinorType) {\n"); lineno++;
  fprintf(out,"  match t {\n"); lineno++;
  fprintf(out,"        Token::EOI => (0, YYMinorType::YY0),\n"); lineno++;
  for(i=1; i<lemp->nterminal; i++){
    if (lemp->symbols[i]->dtnum)
      fprintf(out,"        Token::%s(x) => (%d, YYMinorType::YY%d(x)),\n",lemp->symbols[i]->name,i,lemp->symbols[i]->dtnum);
    else
      fprintf(out,"        Token::%s => (%d, YYMinorType::YY0),\n",lemp->symbols[i]->name,i);
    lineno++;
  }
  fprintf(out,"  }\n"); lineno++;
  fprintf(out,"}\n"); lineno++;
  ...
  fprintf(out,"const YY_ACTTAB_COUNT: i32 = %d;\n", n); lineno++;
  fprintf(out,"const YY_ACTION: [YYACTIONTYPE; %d] = [\n", n); lineno++;
  for(i=j=0; i<n; i++){
  ...
  fprintf(out,"const YY_LOOKAHEAD: [YYCODETYPE; %d] = [\n", n); lineno++;
  for(i=j=0; i<n; i++){
  ...
  fprintf(out, "const YY_SHIFT_USE_DFLT: i32 = %d;\n", mnTknOfst-1); lineno++;
  n = lemp->nstate;
  while( n>0 && lemp->sorted[n-1]->iTknOfst==NO_OFFSET ) n--;
  fprintf(out, "const YY_SHIFT_COUNT: i32 = %d;\n", n-1); lineno++;
  fprintf(out, "const YY_SHIFT_MIN: i32 = %d;\n", mnTknOfst); lineno++;
  fprintf(out, "const YY_SHIFT_MAX: i32 = %d;\n", mxTknOfst); lineno++;
  fprintf(out, "const YY_SHIFT_OFST: [%s; %d] = [\n", 
          minimum_size_type(mnTknOfst-1, mxTknOfst),n); lineno++;
  for(i=j=0; i<n; i++){
    int ofst;
    stp = lemp->sorted[i];
    ofst = stp->iTknOfst;
  ...
  fprintf(out, "const YY_REDUCE_USE_DFLT: i32 = %d;\n", mnNtOfst-1); lineno++;
  n = lemp->nstate;
  while( n>0 && lemp->sorted[n-1]->iNtOfst==NO_OFFSET ) n--;
  fprintf(out, "const YY_REDUCE_COUNT: i32 = %d;\n", n-1); lineno++;
  fprintf(out, "const YY_REDUCE_MIN: i32 = %d;\n", mnNtOfst); lineno++;
  fprintf(out, "const YY_REDUCE_MAX: i32 = %d;\n", mxNtOfst); lineno++;
  fprintf(out, "const YY_REDUCE_OFST: [%s; %d] = [\n", 
          minimum_size_type(mnNtOfst-1, mxNtOfst),n); lineno++;
  for(i=j=0; i<n; i++){
    int ofst;
    stp = lemp->sorted[i];
    ofst = stp->iNtOfst;
  ...
  n = lemp->nstate;
  fprintf(out, "const YY_DEFAULT: [YYACTIONTYPE; %d] = [\n", n); lineno++;
  for(i=j=0; i<n; i++){
    stp = lemp->sorted[i];
    if( j==0 ) fprintf(out," /* %5d */ ", i);
  ...
  */
  ...
  for(i=0; i<mx; i++){
    ...
    if( p->fallback==0 ){
      ...
      if (p->fallback->dtnum != 0 && p->fallback->dtnum != p->dtnum) {
        ErrorMsg(lemp->filename, 0,
          "Fallback token %s must have the same type or no type at all", p->fallback->name);
        lemp->errorcnt++;
      }
    }
    lineno++;
  ...
  */
  fprintf(out,"const YY_RULE_INFO: [YYCODETYPE; %d] = [\n",lemp->nrule); lineno++;
  for(rp=lemp->rule; rp; rp=rp->next){
    fprintf(out,"  %d,\n",rp->lhs->index); lineno++;
  }
  fprintf(out,"];\n"); lineno++;
